...
// Smalltalk-2026: this copy is served by the back end itself (RestServer's
// document root), so the stamp Kiss's build would apply is applied here by
// hand.  With it true, index.js uses the page's own origin whatever the port.
// For the two-server layout (Kiss's static server on port 8000, this server on
// 8001) set it back to false, or set SystemInfo.backendUrl.  demo/README.md.
SystemInfo.sameOriginBackend = true;  // stamped by hand - see above
