...
A C11 compiler, \ct{make}, and SDL3 with its headers. Everything else is in
the tree. A machine that is \emph{meant} to have no display says so and
builds without SDL3: the graphics layer becomes a stub, \ct{-bootstrap},
\ct{-eval}, \ct{-doctests} and \ct{make test} all still work, and
\ct{./st80 -run} refuses to open a window and says why. That is the build
the test suites use and the one a server wants. ODBC and OpenSSL are
optional in a softer way, needing no flag when they are absent: without the
...
\ct{make} stops before it compiles anything if the compiler, the C library
or SDL3 is not there, and the message that stops it carries the command that
installs the missing piece. For SDL3 that is \ct{sudo dnf install SDL3-devel}
on Fedora and \ct{sudo apt install libsdl3-dev} on Debian and Ubuntu. To
build with no display instead, ask for it:

\begin{sh}
make HEADLESS=1
\end{sh}

\ct{make deps} answers what this machine has and what it is missing, one
line per requirement, and runs on a machine too bare to build anything:
...
\ctp{-run image [n]} & run an image, opening a window. \ctp{n} is a bytecode
  limit\\
