...
\begin{stnote}[Which dialect you get]
Full closures are compiled in the \ct{closures} dialect, which every profile
in \ct{profiles/} except \ct{bluebook} selects---for its packages and for
whatever you hand to \ct{-eval} alike. Build with
\ct{-manifest sources/MANIFEST} or \ct{profiles/bluebook.profile} and you
get the 1983 dialect instead, where blocks are \ct{BlockContext} objects
with the 1983 restrictions. Accepting a method in the Browser, loading a
Tonel file into a running server and evaluating in a workspace all reach the
same compiler, so a block means the same thing wherever it was compiled.
\end{stnote}
