...
\ct{Object>>error:} signals an \ct{Error}, so every one of the hundreds of
1983 library methods that calls it raises something you can catch, and an
unhandled one opens the debugger.
...
\ctp{e retryUsing: aBlock} & the same, with a different block: \ctp{aBlock}
  becomes the protected one and the handler stays in place for it\\
...
\ctp{e outer} & run the enclosing handler and, if it resumes, carry on
  here with its value\\
\ctp{e resignalAs: another} & look again from the point of the signal, for
  \ctp{another} instead of this one\\
...
InsufficientFunds >> requested: anAmount available: anotherAmount
    requested := anAmount.
    available := anotherAmount
