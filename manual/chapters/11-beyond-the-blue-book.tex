...
Braces, expressions separated by periods, evaluated at run time. The
elements are compiled in order and one bytecode of the closure set then
builds the Array from them off the stack, so an element costs no literal
and a brace may hold up to 127 of them.
...
(\ct{@}) are rejected with a report rather than silently mis-loaded. A class
answers \ct{traitComposition} with the text its header gave---for
\ct{Greeter} that is \ct{'TGreeting'}---and the flattened methods sit in a
\ct{*trait:TGreeting} category, which is all the reflection there is: there
is no update propagation, because flattening is a load-time operation, and
changing a trait afterwards does not change the classes that took it. A
class file loaded at run time through \ct{TonelReader} (Chapter~\ref{ch:packages})
cannot name a trait, since the trait's source lives in the bootstrap.
