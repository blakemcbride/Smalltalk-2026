...
\ctp{addFirst:} \ctp{addLast:} \ctp{removeFirst} \ctp{removeLast} & ordered only\\
\midrule
...
The forty-years-of-protocol methods this system adds---\ct{sorted:},
\ct{flatCollect:}, \ct{count:} and the rest---answer an \ct{Array} where Pharo
would answer \ct{self species}, except for \ct{sorted} and \ct{sorted:} on a
sequenceable collection, where order is the point and the species is obvious.
A sorted Set is not a Set and a flattened Bag is not a Bag, so \ct{species} is
already the wrong answer for two of the three interesting receivers. The
methods that \emph{preserve} shape---\ct{select:}, \ct{reject:},
\ct{collect:}, \ct{copyWithout:}---go on using \ct{species}, because there the
1983 answer was already right.
\end{stnote}
...
d values                         "an Array of the values"
