...
Three limits are refusals of a different kind. A document may not nest
deeper than 512 levels, an exponent may not exceed 4096, and a number may
not be written with more than 256 digits---the first so that forty thousand
open brackets cannot exhaust the heap, the second so that a twelve-character
document cannot ask this process for a billion digits, the third because
reading a decimal into a binary integer costs a multiplication per digit of
a number that grows. All three matter because the document usually arrives
from somewhere else. 256 digits is past every number that is really a
number: a 64-bit integer is 19 digits, the widest SQL \ct{DECIMAL} is 38,
and a double carries 17 significant ones. The count is taken separately in
the whole part and in the fraction.

Two of the three can be raised deliberately:

\begin{st}
| parser |
parser := JSONParser on: aVeryDeepDocument.
parser maxDepth: 5000.
parser maxDigits: 2000.
parser parse
\end{st}
...
\ctp{JSONParser on:} & a parser, for maxDepth:, maxDigits: and nextValue \\
\midrule
\textbf{Writing} & \\
