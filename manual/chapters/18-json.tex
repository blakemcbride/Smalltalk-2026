...
\begin{stwarn}
An object with thousands of \emph{names} is quadratic to build, and it is not
this package's fault: 1983's \ct{String>>hash} reads the first character, the
second-to-last, and the length, so two hundred names like \ct{key1}..\ct{key200}
share eleven hash values and the \ct{Dictionary} degenerates into one long
chain. A few dozen names---what almost every document has---is unaffected. See
\ct{doc/JSON.md}.
\end{stwarn}
