...
\item \textbf{Bytecode-level memory safety.} The object memory never tears. A
  ...
  always consistent.
\item \textbf{\ct{become:} is atomic.} Identity mutation is two
  object-table stores, made at a safepoint, so no thread ever observes a
  half-completed \ct{become:}. What keeping an object table buys is that the
  safepoint is O(1) rather than a stop-the-world scan of every reference in
  the heap.
...
as a gate rather than a report, and it is what \ct{./st80 -serve image}
runs (Chapter~\ref{ch:serving}). It is \emph{not} yet what
