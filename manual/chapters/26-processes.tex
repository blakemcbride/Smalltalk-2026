...
All three of \ct{terminate}, \ct{suspend} and \ct{signalException:} reach a
process wherever it is: on a ready list, waiting on a \ct{Semaphore}, or
running on another worker, in which case that worker parks it at its next
bytecode and the sender waits for that before going on. \ct{terminate} runs
every \ct{ensure:} block the process is inside, innermost first, on the
process that sends \ct{terminate}---so a process terminated inside
\ct{aMutex critical:} releases the mutex on its way out---and leaves
\ct{suspendedContext} nil, which is what a terminated process looks like.
\ct{resume} refuses anything but a process that is parked and free: a
terminated one, one already waiting on a list, and one that is running
(\ct{Processor activeProcess resume} included) each raise an error naming
the reason. \ct{suspend} refuses a process waiting on a \ct{Semaphore},
because resumed from there it would go on past the wait as though the signal
had come.
...
A duration is rounded to the millisecond when the \ct{Delay} is made, and a
negative one is zero. The timer counts up to \ct{Delay maximumMilliseconds},
$2^{29}-1$ milliseconds or a little over six days; a longer delay, or one
made from something that is not a number, is refused there with an error
that says so, rather than discovered inside \ct{wait}.
...
The REST server's gates (Chapter~\ref{ch:serving}) added a third shape.
The timing process that resumes Delays is the one process every worker runs
in turn, and it lost a reference count about once a minute on 31 workers:
the scheduler's \ct{activeProcess} slot is written by every worker on every
switch, and a plain store---read the old value, store the new, release the
old---lets two workers release the same old occupant. The slot is exchanged
atomically now. When every \ct{Delay} in a system stops waking at once, the
timing process is the first thing to look for.
\end{stnote}
...
\ct{Time now} and \ct{Date today} answer the time where the machine is. The
offset comes from the host's own zone database, asked for at the moment it
is needed, so a rule that changes twice a year is right on both sides of the
change.

\ct{Time millisecondClockValue} is a monotonic count of milliseconds---it
never goes backwards, and it does not wrap, so two readings may simply be
subtracted however far apart they are. It counts from an arbitrary moment
and says nothing about the date; \ct{Time now} is for that.

\begin{stwarn}
\ct{Time} and \ct{Date} are 1983's in the \ct{st2026} profile. The
...
passes its own 621 tests. Which you have depends on the profile you built.
Both keep the same zone.
\end{stwarn}
