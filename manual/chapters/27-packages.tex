...
TonelSource loadFile: 'lib/MyStuff/Widget.class.st'.     "the same, and remember where it came from"
TonelSource reloadIfChanged: Widget.                     "read it again if the file has changed"
...
not a notifier. Loading a class that already exists redefines it in place
and removes the methods the file no longer has. \ct{TonelSource} adds the
registry: a class it loaded is written back to its file when a method is
accepted or removed in the Browser (Chapter~\ref{ch:keeping}), and
\ct{reloadIfChanged:} compares the file's time and size with what it saw
last. \ct{TonelWriter} writes what the reader reads, so a class can go out
