...
Reentrant means that a \ct{critical:} sent from inside another on the same
monitor, by the same process, goes one level deeper rather than waiting for
itself; the monitor is released when the outermost block ends, however it
ends. \ct{waitForChange} and \ct{waitUntil:} give up the whole depth while
they wait---a waiter two levels in still lets everyone else in---and take it
back with the monitor. \ct{isHeldByActiveProcess} answers whether the process
asking holds it, which is what a method that must be sent from inside
\ct{critical:} can assert.
...
\begin{stwarn}[Scratch space in a class variable]
\begin{st}
Buffer at: (i := i + 1) put: digit       "every instance, one Buffer"
\end{st}
Correct under a green scheduler, because nothing of the same priority can
run between the write and the read back. Here every worker writes into the
same slots. 1983's \ct{SmallInteger>>printOn:base:} did exactly this, and
eight workers printing integers read back each other's digits. Put the
buffer in a temporary.
\end{stwarn}

\begin{stwarn}[Reading a shared cache twice]
\begin{st}
Cache isNil ifTrue: [^self rebuild].
^Cache                                   "nil, if somebody flushed it"
\end{st}
Read it once into a temporary and use that. 1983's \ct{TempNameCache} and
\ct{CachedClassNames} both did this.
\end{stwarn}

\begin{stnote}
The 1983 structures that every program shares are guarded for you:
\ct{Symbol intern:}, \ct{Smalltalk at:put:} and its readers, the dependents
table behind \ct{addDependent:}, the list of open directories, every method
dictionary and each send to the \ct{Transcript} all take a lock from
\ct{LibraryLocks}. \ct{Compiler evaluate:}
runs the compiled method without installing it, \ct{Processor yield} is a
primitive, and a \ct{Delay} survives a stale timer signal.
\ct{doc/CONCURRENCY.md} has the table.
\end{stnote}
...
parallel test suites, \ct{make bench} and \ct{st80 -serve} that run on many
native threads today (Chapter~\ref{ch:serving}). Write to the contract regardless: the code is the same either way, and
