...
\section{A class from a file, at run time}

A profile is read by the C bootstrap. The same files can be read by the
running image, which is what the REST server does with its services
(Chapter~\ref{ch:serving}) and what you can do with any class:

\begin{st}
TonelReader loadFile: 'lib/MyStuff/Widget.class.st'.     "compile it in; answer the class"
TonelSource loadFile: 'lib/MyStuff/Widget.class.st'.     "the same, remembering the file"
TonelSource reloadIfChanged: Widget.                     "read it again if it has changed"
TonelWriter write: Widget toFile: 'lib/MyStuff/Widget.class.st'.
\end{st}

\ct{TonelReader} reads a class or extension file exactly as the bootstrap
does---the same grammar, the same \ct|{ #category : ... }| headings---and
compiles each method through the same compiler the Browser uses, so a syntax
error is a \ct{TonelError} naming the file, the method and the character,
not a notifier. Every method in the file is compiled before any is
installed, so a file with a mistake in it changes nothing. Loading a class
that already exists redefines it in place and removes the methods the file
no longer has---its own methods, that is: a method whose category begins with
a star belongs to another package's extension file, or to a trait, and is
left alone. \ct{TonelSource} adds the registry: a class it loaded is written
back to its file when a method is accepted or removed in the Browser
(Chapter~\ref{ch:keeping}), and \ct{reloadIfChanged:} compares the file's
time and size with what it saw last. A class loaded from an extension file
is written back to that file as an extension, holding the package's methods
and no others. \ct{TonelWriter} writes what the reader reads---a weak class as
\ct|#type : 'weak'|, a class that takes a trait as \ctp{\#traits} without
the trait's methods---so a class can go out and come back byte for byte.
...
\begin{plain}
st80: 151 classes were given "new ^super new initialize": ...
st80: 380 classes, 7599 methods, 5581 symbols
st80: 81 undeclared globals: Sensor Disk TextConstants ... and 69 more
st80: 6 undeclared names resolved from a pool dictionary
st80: 53 class initializers, 50 ran, 3 skipped, 0 unfinished;
      5691 of 5691 symbols in the library table
\end{plain}
...
\item[undeclared globals] names referenced but never defined. Most are
  expected: \ct{Sensor}, \ct{Display} and \ct{Transcript} are made when the
  image is built, long after the code that names them is compiled. A
  \emph{lower-case} one is a block argument used outside its block, and gets
  a line of its own naming the methods that still read it.
...
\item[symbols in the library table] every Symbol the bootstrap interned,
  placed in the image's own symbol table. The two numbers have to be the
  same: a Symbol missing from the table can be interned a second time, and
  then two of it exist and \ct{==} is no longer the whole story.
\end{description}
