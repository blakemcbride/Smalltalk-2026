...
\ctp{-serve <image> [-workers n] [args]} & run the image on \ctp{n} native threads (four per CPU by default, and never more than 63), no window, until SIGINT, SIGTERM or \ctp{Smalltalk quitPrimitive}; \ctp{args} are what \ctp{Smalltalk arguments} answers. The image's startup is what \ctp{-bootstrap -startup} gave it---Chapter~\ref{ch:serving}\\
...
\section{What it answers the shell}

Zero when the thing asked for was done, and when nothing was asked for:
\ct{-version}, \ct{-help} and a bare \ct{./st80} print and succeed.

One when the work was attempted and failed---an image that cannot be opened,
a bootstrap that will not compile, \ct{-tests} with a test that did not pass,
an \ct{-eval} whose expression ended in an unhandled error.

Two when the command line itself is wrong: an option this system does not
have, or one whose argument is missing. The message names the option and goes
to standard error, where a script redirecting output will still see it.
