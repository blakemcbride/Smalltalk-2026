...
\item[A service will not load: \ctp{... is undeclared}] A name in the
  file that is neither a temporary, an instance variable nor a global. If
  it is a block-local temporary, \ct{[:each | | row | ...]}, move it to the
  method: the image's own compiler is 1983's and has none.
\end{description}
