...
The next thing the system wants is for you to say where it goes: press the red
button, drag out a rectangle, release. The window appears when you let go.
...
\begin{tabular}{@{}lll@{}}
\toprule
colour & button & what it does\\
\midrule
red    & left   & select: text, list items, everything\\
yellow & middle & the \emph{view's} own menu---what you can do to the contents\\
blue   & right  & the \emph{window's} menu---move it, close it\\
\bottomrule
\end{tabular}
\end{center}

If you have a two-button mouse or a trackpad, you will need whatever your
system offers for a middle click. Yellow is the button you use most.
...
\begin{enumerate}
\item Press and \emph{hold} the \textbf{yellow} (middle) button anywhere on
...
\item The cursor becomes a corner. Press the \textbf{red} (left) button
...
\caption{The desktop menu, held open with the yellow button. The grey
...
Now select it---press red at the start, drag to the end, release---and with
the text still selected press and hold the \textbf{yellow} button. This menu
...
the yellow-button menu and nowhere else, and this system is that system.
\end{stnote}
...
There is no title bar to drag and no close box. Hold the \textbf{blue} (right)
...
Same gesture: yellow on the desktop, drag to \ct{browser}, release, then frame
...
Click along the top panes with the red button: pick a category on the left,
...
the yellow-button menu compiles it back into the running system.
...
Yellow on the desktop, then \ct{quit}, which offers three choices: save and
