...
\ctp{e description} & a fixed sentence for the class ---
  \ctp{'An error has occurred'} for \ctp{Error}. It does \emph{not} answer the
  message text\\
...
with. \ct{([Error signal: 'a'] on: Error do: [:e | e description])} answers
\ct{'An error has occurred'}. You almost always want \ct{messageText}, which
falls back to \ct{description} when there is no text.
\end{stwarn}
