...
\begin{stwarn}[A doit is not compiled in the profile's dialect]
\ct|#dialect| governs the \emph{packages} a profile loads. The expression
you hand to \ct{-eval}, and each example \ct{-doctests} runs, is compiled
separately and defaults to the Blue Book dialect however the profile was
written. The visible symptom is that \ct{[:x | x] class} answers
\ct{BlockContext} rather than \ct{BlockClosure}. Pass \ct{-closures} on the
command line to compile doits the same way:

\begin{sh}
./st80 -bootstrap -profile profiles/st2026.profile -closures -eval '[:x | x] class'
\end{sh}
\end{stwarn}
...
\ctp{new} $\rightarrow$ \ctp{initialize} & for loaded packages, synthesised
