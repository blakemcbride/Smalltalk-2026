...
\begin{tabular}{@{}ll@{}}
\toprule
\ctp{abs} \ctp{negated} \ctp{sign} & $|x|$, $-x$, and $-1$/$0$/$1$\\
...
\begin{stnote}[Two that are not here]
\ct{printString: aBase} and \ct{radix:} are Pharo's and this library does not
have them---\ct{255 printString: 16} is a doesNotUnderstand. What is here is
\ct{printOn:base:length:padded:} and \ct{printStringPadded:}, from the
protocol shims in \ct{lib/Kernel-Protocol}. Radix \emph{literals} (\ct{16rFF})
work fine; it is only printing in a base that is missing.
\end{stnote}
